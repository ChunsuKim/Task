...
func leafYear ( _ year: Int) {
    var count = 0
    for i in 1...year {
        let isLeafYear = i % 4 == 0 && (i % 100 != 0 || i % 400 == 0)
        count += isLeafYear ? 1 : 0        // true ? print(1) : print(0)
    }
...
let inputArr = ["a", "b", "c", "a", "e", "d", "c"]

func notSameCharacter (from arr: [String]) -> [String] {
...
print(notSameCharacter(from: inputArr))
...
func checkPrimeNumbers(number: Int) {
    ...
    for i in 2...number {
        ...
        for j in 2..<Int(sqrt(Double(i)) + 1) {         // sqrt = 제곱근
            if i % j == 0 {
    ...
    print("\(number)보다 작거나 같은 소수는 \(primeNumbers)이고, 총 \(primeNumbers.count) 입니다.")
}
