...
class ViewController: UIViewController {
  ...
  @IBAction private func bundleFile(_ sender: UIButton) {
    ...
    let melonChart = songsChart["Melon"] as? [String: [String]] else { return }
  ...
  @IBAction private func updateFileData(_ sender: Any) {
    ...
    guard let filePath = bundle.path(forResource: "SongsChart", ofType: "plist"),
        let fileData = fileManager.contents(atPath: filePath),
        var songsChart = try? PropertyListSerialization.propertyList(from: fileData, format: nil) as? [String: [String: [String]]] else { return }
  ...
  private func saveFileUrl() -> URL {
    let appSupportDir = fileManager.urls(for: .applicationDirectory, in: .userDomainMask).first!
